...
wsrep_create_streaming_applier(THD *orig_thd, const char *ctx, XID* xid)
{
      ...
      (thd= new THD(next_thread_id(), true)))
  {
    ...
    if (xid && !xid->is_null())
    {
      if (orig_thd->wsrep_trx().state() == wsrep::transaction::s_replaying)
      {
        ret= new (std::nothrow) Wsrep_prepared_replayer_service(thd, orig_thd, xid);
      }
      else
      {
  ...
  wsrep::client_service& orig_client_service)
{
  ...
  XID xid;
  xid.null();
  if (orig_cs.m_thd->lex->sql_command != SQLCOM_XA_ROLLBACK &&
      (orig_cs.m_thd->wsrep_trx().state() == wsrep::transaction::s_prepared ||
       orig_cs.m_thd->wsrep_trx().state() == wsrep::transaction::s_replaying))
  {
    xid= static_cast<const Wsrep_xid&>(orig_cs.m_thd->wsrep_trx().xid());
  }
