...
void wsrep_delete_threadvars()
{
  if (thread_handling == SCHEDULER_TYPES_COUNT)
  {
    /* The caller should have called wsrep_store_threadvars() before
       this method. */
    DBUG_ASSERT(pthread_getspecific(THR_KEY_mysys));
    /* We should never end here with PSI per-client structure set,
       as streaming appliers are deleted only from applier context.
       However, reset PSI_thread for duration of my_thread_end() just
       in case to avoid memory corruption. */
    PSI_thread *psi_thread= PSI_CALL_get_thread();
    DBUG_ASSERT(!psi_thread);
    if (PSI_server)
    {
