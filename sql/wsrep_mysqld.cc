...
int Wsrep_gtid_server::wait_gtid_upto(THD *thd, const uint64_t seqno,
...

{
  ...
  if (seqno > m_seqno)
  {
    ...
    {
      /* If wait_time is NULL, waits indefinitely. We should the
       * wait in small increments if wait_time is NULL and check if killed */
      struct timespec max_wait_time;
      set_timespec(max_wait_time, 100000);
      struct timespec *wait_time_ptr= wait_time ? wait_time : &max_wait_time;
      wait_result= mysql_cond_timedwait(&wait_cond, &LOCK_wsrep_gtid_wait_upto,
                                        wait_time_ptr);
      if (!wait_time && thd && thd->killed)
      {
        ret= EINTR;
        break;
      }
      else if (wait_result == ETIMEDOUT || wait_result == ETIME)
      {
        ret= wait_result;
        break;
      }
