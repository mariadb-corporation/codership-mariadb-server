...
@param[out] space  bytes left for the value of this column, the null flag
                   already taken off
@return false if not even the flag byte (SQL NULL) or one byte of the value
        (otherwise) fits, true otherwise */
static
bool
	...
	ulint*			space)
{
	...
	const ulint	needed = flag_len + (len == UNIV_SQL_NULL ? 0 : 1);
